...
def biLSTM(input_units, n_hidden_list, sequence_length):
	units = input_units
	for n, n_h in enumerate(n_hidden_list):
		with tf.variable_scope('LSTM' + str(n)):
			...
			(rnn_output_fw, rnn_output_bw), _ = tf.nn.bidirectional_dynamic_rnn(forward_cell, backward_cell, units, 
...
def character_embedding_network(char_placeholder, n_characters, char_embedding_dim, filter_width=3):
	...
	with tf.variable_scope('CharEmbNetwork'):
		...
		#char_emb = tf.nn.max_pool(char_conv, (1, reduced_length, 1, 1), (1, 1, 1, 1), padding='VALID')
